	...
	<div class="container">
		...
		<div class= "container">
			<ul class="pagination justify-content-center">
				...
				<c:if test="${pag != 0 }">
					<c:if test="${pag != 1}"><li class="page-item"><a href="pdsList.pds?pag=1&part=${part}" title="첫페이지" class="page-link text-secondary">◁◁</a></li></c:if>
					<c:if test="${curBlock > 0}"><li class="page-item"><a href="pdsList.pds?pag=${(curBlock-1)*blockSize + 1}&part=${part}" title="이전블록" class="page-link text-secondary">◀</a></li></c:if>
					<c:forEach var="i" begin="${(curBlock*blockSize)+1 }" end="${(curBlock*blockSize)+blockSize }">
						<c:if test="${i == pag && i <= totPage }"><li class="page-item active"><a href='pdsList.pds?pag=${i }&part=${part}' class="page-link bg-secondary border-secondary">${i }</a></li></c:if>
						<c:if test="${i != pag && i <= totPage }"><li class="page-item"><a href='pdsList.pds?pag=${i }&part=${part}' class="page-link text-secondary">${i }</a></li></c:if>
					</c:forEach>
					<c:if test="${curBlock < lastBlock}"><li class="page-item"><a href="pdsList.pds?pag=${(curBlock+1)*blockSize + 1}&part=${part}" title="다음블록" class="page-link text-secondary">▶</a></li></c:if>
					<c:if test="${pag != totPage}"><li class="page-item"><a href="pdsList.pds?pag=${totPage}&part=${part}" title="마지막페이지" class="page-link text-secondary">▷▷</a></li></c:if>
				</c:if>
			</ul>
		</div>
	</div>
